...
__attribute__((naked)) void switch_context(uint32_t* prev_sp,
    ...
    __asm__ __volatile__(
        ...
        "addi sp, sp, -13 * 4\n"  // Allocate stack space for 13 4B registers
...
struct process procs[PROCS_MAX];  // All processes control structures.
...
struct process* create_process(uint32_t pc) {
    ...
    // Stack callee-saved registers. These register values will be
    // restored in the first context switch in switch_context.
...
__attribute__((section(".text.boot"))) __attribute__((naked)) void boot(void) {
    __asm__ __volatile__(
        ...
        :                               // output operands
        : [stack_top] "r"(__stack_top)  // input operands: pass __stack_top
                                        // as %[stack_top]
    );
}